...
class ResourceToolBox extends ToolBox {
    ...
    updateDependency(ev, button, activity, action) {
        ...
        this.sendRequest(data, spinner, 'mod_quiz_update_dependency').then(response => {
...
class SectionToolBox extends ToolBox {
    ...
    constructor(courseId, quizId, ajaxURL) {
        ...
        this.ajaxURL = ajaxURL;
        this.config = config;
        M.mod_quiz.quizbase.registerModule(this);
        ...
        document.body.addEventListener('click', (event) => {
            if (event.target.matches(SELECTOR.ACTIVITYACTION)) {
                this.handleDataAction(event);
            }
        });

        document.body.addEventListener('change', (event) => {
            if (event.target.matches(SELECTOR.EDITSHUFFLEQUESTIONSACTION)) {
                this.handleDataAction(event);
            }
        });
    }
    ...
    handleDataAction(ev) {
        ...
        switch (action) {
            case 'edit_section_title':
                ...
                this.editSectionTitle(ev, node, activity, action);
                break;
            case 'shuffle_questions':
                // The user wishes to edit the shuffle questions of the section (resource).
                this.editShuffleQuestions(ev, node, activity, action);
                break;
            case 'deletesection':
                // The user is deleting the activity.
                this.deleteSectionWithConfirmation(ev, node, activity, action);
                break;
            default:
                // Nothing to do here!
                break;
    ...
    async deleteSectionWithConfirmation(ev, button, activity) {
        ev.preventDefault();
        const [title, question, saveLabel] = getStrings([
            {key: 'confirm', component: 'moodle'},
        ...
        Notification.saveCancelPromise(
            ...
            const spinner = util.addSpinner(activity.querySelector(SELECTOR.ACTIONAREA));
            const data = {
                'class': 'section',
                'action': 'DELETE',
                'id': activity.id.replace('section-', '')
            };
            this.send_request(data, spinner, (response) => {
                if (response.deleted) {
    ...
    editSectionTitle(ev, button, activity) {
        ...
        this.send_request(data, null, (response) => {
            // Try to retrieve the existing string from the server.
            ...
            editInstructions.classList.add(CSS.EDITINSTRUCTIONS);
            editInstructions.setAttribute('id', 'id_editinstructions');
            editInstructions.innerHTML = M.util.get_string('edittitleinstructions', 'moodle');
            const editor = document.createElement('input');
            editor.setAttribute('name', 'section');
            editor.setAttribute('type', 'text');
            editor.setAttribute('value', oldText);
            editor.setAttribute('autocomplete', 'off');
            editor.setAttribute('aria-describedby', 'id_editinstructions');
            editor.setAttribute('maxLength', '255'); // This is the maxlength in DB.

            // Clear the existing content and put the editor in.
            editForm.appendChild(editor);
            editForm.dataset.anchor = anchor;
            instanceSection.parentNode.insertBefore(editInstructions, instanceSection);
            anchor.parentNode.replaceChild(editForm, anchor);

            // Focus and select the editor text.
            editor.focus();
            editor.select();
            // Cancel the edit if we lose focus or the escape key is pressed.
            editor.addEventListener('blur', () => this.edit_section_title_cancel(activity, false));
            editor.addEventListener('keydown', (event) => {
                if (event.key === 'Escape') {
                    this.edit_section_title_cancel(activity, true);
                }
            });
            // Handle form submission.
            editForm.addEventListener('submit', (event) => this.edit_section_title_submit(event, activity, oldText));
        });
    }
     ...
     * @param {Event} ev The event that triggered this.
     * @param {Node} activity The activity whose section heading we are editing.
     * @param {String} oldText The original section heading.
     */
    editSectionTitleSubmit(ev, activity, oldText) {
        // We don't actually want to submit anything.
        ev.preventDefault();
        const newTextInput = activity.querySelector(`${SELECTOR.SECTIONFORM} ${SELECTOR.SECTIONINPUT}`);
        let newtext = Y.Lang.trim(newTextInput.value);
        const spinner = M.util.add_spinner(Y, activity.querySelector(SELECTOR.INSTANCESECTIONAREA));
        ...
        this.send_request(data, spinner, (response) => {
            if (response) {
                ...
                editSectionIcon.title = M.util.get_string('sectionheadingedit', 'quiz', response.instancesection);
                editSectionIcon.alt = M.util.get_string('sectionheadingedit', 'quiz', response.instancesection);

                const deleteIcon = activity.querySelector(SELECTOR.DELETESECTIONICON);
                if (deleteIcon) {
                    deleteIcon.title = M.util.get_string('sectionheadingremove', 'quiz', response.instancesection);
                    deleteIcon.alt = M.util.get_string('sectionheadingremove', 'quiz', response.instancesection);
                }
            }
        });
    },
     ...
     * @param {Node} activity The activity whose section heading we are editing.
     */
    editSectionTitleCancel(ev, activity) {
        ev.preventDefault();
        this.editSectionTitleClear(activity);
    }
     ...
     * @param {Node} activity The activity whose section heading we were editing.
     */
    editSectionTitleClear(activity) {
        // Detach all listen events to prevent duplicate triggers
        this.editSectionEvents.forEach(event => event.detach());

        const editForm = activity.querySelector(SELECTOR.SECTIONFORM);
        const instructions = activity.querySelector('#id_editinstructions');
        if (editForm) {
            editForm.replaceWith(editForm.dataset.anchor);
        }
        if (instructions) {
            instructions.remove();
        }
        ...
        if (!document.querySelector('input[name=section]')) {
            document.querySelector('body').insertAdjacentHTML('beforeend', '<input type="text" name="section" style="display: none">');
        }
    }
    ...
    editShuffleQuestions(ev, button, activity) {
        ...
        // Send request.
        const spinner = M.util.add_spinner(Y, activity.one(SELECTOR.EDITSHUFFLEAREA));
...
function initResourceToolbox(courseid, quizid, ajaxurl) {
    new ResourceToolBox(courseid, quizid, ajaxurl);
}

/**
 *
 */
function initSectionToolbox(courseid, quizid, ajaxurl) {
    new SectionToolBox(courseid, quizid, ajaxurl);
}
const config = JSON.parse(document.querySelector(SELECTOR.CONFIGTOOLBOX).dataset.config);
